...
func cleanWorkDir() {
	if removeTempWorkDir {
		...
		if err != nil {
			_ = fmt.Errorf("Error: Can not remove cpu profile '%s': %s\n",
				getAbsFilePath(cpuProfilePath), err)
		}
		err = remove(blockProfilePath, 5, 2)
		if err != nil {
			_ = fmt.Errorf("Error: Can not remove block profile '%s': %s\n",
				getAbsFilePath(blockProfilePath), err)
		}
		err = remove(memProfilePath, 5, 2)
		if err != nil {
			_ = fmt.Errorf("Error: Can not remove mem profile '%s': %s\n",
	...
	if newTempWorkDir && removeTempWorkDir {
		...
		if err != nil {
			_ = fmt.Errorf("Error: Can not remove temp work dir '%s': %s\n",

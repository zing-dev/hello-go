...
func TestReadGCStats(t *testing.T) {
	...
	if stats.NumGC != int64(mstats.NumGC) {
		...
		t.Logf("stats.NumGC = %d, mstats.NumGC = %d", stats.NumGC, mstats.NumGC)
	}
	...
	if stats.PauseTotal != time.Duration(mstats.PauseTotalNs) {
		...
		t.Logf("stats.PauseTotal = %d, mstats.PauseTotalNs = %d", stats.PauseTotal, mstats.PauseTotalNs)
	}
	...
	if stats.LastGC.UnixNano() != int64(mstats.LastGC) {
		...
		t.Logf("stats.LastGC.UnixNano = %d, mstats.LastGC = %d", stats.LastGC.UnixNano(), mstats.LastGC)
	}
	...
	if len(stats.Pause) != n {
		...
		for i := 0; i < n; i++ {
			...
			if dt != time.Duration(mstats.PauseNs[off]) {
				...
				t.Logf("stats.Pause[%d] = %d, want %d", i, dt, mstats.PauseNs[off])
			}
			if max < dt {
				max = dt
			}
	...
	if q[0] != min || q[nq-1] != max {
		...
		t.Logf("stats.PauseQuantiles = [%d, ..., %d], want [%d, ..., %d]", q[0], q[nq-1], min, max)
	}
	...
	for i := 0; i < nq-1; i++ {
		if q[i] > q[i+1] {
			...
			t.Logf("stats.PauseQuantiles[%d]=%d > stats.PauseQuantiles[%d]=%d", i, q[i], i+1, q[i+1])
		}
	}
...
func TestFreeOSMemory(t *testing.T) {
	...
	if ms2.HeapReleased >= ms3.HeapReleased {
		t.Errorf("released before=%d; released after=%d; did not go up", ms1.HeapReleased, ms2.HeapReleased)
	} else {
		t.Logf("released before=%d,%d; released after=%d; did not go up", ms1.HeapReleased/1024, ms2.HeapReleased/1024, ms3.HeapReleased/1024)
	}
	fmt.Println(1 << 20)
	fmt.Println(ms3.HeapReleased - ms2.HeapReleased)
}

...
func startCPUProfile() {
	if *cpuProfile != "" {
		...
		if err != nil {
			_, _ = fmt.Fprintf(os.Stderr, "Can not create cpu profile output file: %s\n", err)
			return
		}
		if err := pprof.StartCPUProfile(f); err != nil {
			_, _ = fmt.Fprintf(os.Stderr, "Can not start cpu profile: %s\n", err)
			_ = f.Close()
			return
		}
	}
...
func stopBlockProfile() {
	if *blockProfile != "" && *blockProfileRate >= 0 {
		...
		if err != nil {
			_, _ = fmt.Fprintf(os.Stderr, "Can not create block profile output file: %s\n", err)
			return
		}
		if err = pprof.Lookup("block").WriteTo(f, 0); err != nil {
			_, _ = fmt.Fprintf(os.Stderr, "Can not write %s: %s\n", *blockProfile, err)
		}
		_ = f.Close()
	}
}
...
func stopMemProfile() {
	if *memProfile != "" {
		...
		if err != nil {
			_, _ = fmt.Fprintf(os.Stderr, "Can not create mem profile output file: %s\n", err)
			return
		}
...
func SaveProfile(workDir string, profileName string, ptype ProfileType, debug int) {
	...
	if err != nil {
		_, _ = fmt.Fprintf(os.Stderr, "Can not create profile output file: %s\n", err)
		return
	}
